...
Codex는 JH 생태계의 독립 검수자다. Claude Code는 구현을 맡고, Codex는 Claude의 판단을 그대로 따르지 않고 독립적으로 확인한 뒤 사용자에게 직접 보고한다.
...
7. Bucky가 항상 대기하지 않는 경우, Codex는 발동 스위치로 `python -X utf8 scripts/context_pack_selector.py "<요청문>"`를 실행해 필요한 Context Pack 후보를 확인한다.
8. 바로 적용 가능한 패킷이 필요하면 `python -X utf8 scripts/context_pack_selector.py --packet --project "<repo-or-folder>" "<요청문>"`를 실행한다.
