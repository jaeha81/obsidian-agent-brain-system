...
## Codex Watch Skill Trigger (Bucky-managed)
...
2. Codex reads `C:\Users\user1\.codex\skills\watch\SKILL.md` as the skill entrypoint and follows the Windows note: use `python`, not `python3`.
...
4. Bucky owns when this capability is included in Codex/Claude Code instruction packets for OABS work.
5. Codex must not create, replace, or relink OS-level skill/plugin paths unless the user explicitly asks for that setup change.

## 3단계 라우팅

Bucky는 모든 작업의 선행 관문이 아니라 필요한 지식/정책 라우터다.

1. **명확한 명령 경로:** 사용자가 파일, 명령, 실행 순서, 금지 작업을 명시하면 첫 명령을 즉시 실행한다. selector, 계획서, 대형 diff, Context Pack 읽기는 먼저 하지 않는다.
2. **일반 구현 경로:** 사용자가 변경을 요청했지만 실행 단계가 정확히 정해지지 않았으면 Claude Code 또는 Codex가 짧은 micro-plan을 먼저 제시하고 사용자 확인을 받는다. 승인 후 필요한 정책이나 프로젝트 지식만 Bucky/context에서 받는다.
3. **Bucky 선행 경로:** 새 프로젝트, 지침 권한 불명확, 보안/인증/결제/배포/고객 데이터 위험, 삭제/이동/reset 같은 파괴적 작업, 광범위 migration, 역할/지침 변경, 또는 사용자가 Bucky 확인을 명시한 경우에만 계획 전 Bucky/selector를 사용한다.
