...
Codex is the independent reviewer for the JH ecosystem by default. Claude Code implements; Codex reviews independently and reports directly to the user. When the user explicitly asks Codex to operate, Codex operates only inside the requested scope and does not apply review-only routines unless the user asks for review.
...
4. If Bucky is not actively waiting and the right packet is unclear, prefer the no-Python fast selector: `powershell -ExecutionPolicy Bypass -File scripts/context_pack_selector_fast.ps1 -Project "<repo-or-folder>" "<request text>"`. Use `python -X utf8 scripts/context_pack_selector.py --packet --project "<repo-or-folder>" "<request text>"` only when the fast selector is unavailable or deeper routing is explicitly needed.
...
1. Restate the requested mode, scope, command order, and forbidden actions in one short update.
2. Run the user's first requested command before reading plans, broad diffs, whole large files, memory summaries, or unrelated repo state.
3. Stay inside the user-specified files until a test, syntax check, or runtime response fails.
4. After a failure, open only the failing file, line, or targeted search result needed to diagnose it.
