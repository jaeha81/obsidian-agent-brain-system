...
class ContextPackSelectorTests(unittest.TestCase):
    def test_select_review_pack_for_review_request(self):
        ...
        self.assertIn("Review Pack", selection["packs"])
        self.assertIn("ObsidianVault/03_Projects/agents/codex-instructions.md", selection["notes"])
    ...
    def test_select_discord_pack_for_discord_pipeline_body(self):
        ...
        self.assertIn("Discord Pipeline Pack", selection["packs"])
        self.assertIn("ObsidianVault/05_Frameworks/guides/discord-fallback-pipeline.md", selection["notes"])
    ...
    def test_select_sync_pack_for_pc_or_storage_body(self):
        ...
        self.assertEqual(selection["primary_worker"], "Sync Sentinel")
        self.assertIn("PC Sync Pack", selection["packs"])
