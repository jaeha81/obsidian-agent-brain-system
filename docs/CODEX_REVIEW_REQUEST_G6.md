...
## 14. Codex 재검수 #4 결과 (2026-07-13) — ✅ **통과(PASS) · G6 종결**

대상: `259a50b`. **필수 결함 0건. Codex가 G6 통과를 명확히 선언했다.**

### 14.1 항목별 판정

| 항목 | 판정 | Codex 독립 재현 |
|---|---|---|
| A. 신규 노트 생성 경합 | **PASS** | 로컬·G: 양쪽에서 12중 동시 게시 → 승자 1 / `FileExistsError` 11 / 최종 본문이 승자 본문과 **완전 일치** / `.tmp` 잔여 0. 기존 대상 게시 시 양쪽 `FileExistsError`·원본 보존. `os.name != "nt"`에서 POSIX `rename` 폴백을 차단한 판단도 적절 |
| B. 회귀 가드 | **PASS** | 각 수정을 되돌리면 정확히 해당 테스트 1건만 실패. "통과 전용 가짜 가드가 아니다" |
| C. 멱등성·never-overwrite | **PASS** | 동일 결과 2회 병합 → 무변경. topics-only 첫 병합 후 재병합도 무변경 |
| D. P1-15 정확성 | **PASS** (비차단 보완점 1) | 09:00 예약·Discord `!수집`·수동 CLI·`--watch` 경로가 코드와 일치. 08:00 근거 철회도 정확 |
| E. 회귀 | **PASS** | 432건 / 19F / 13E, provenance 17건 OK — 주장 수치 재현 |
| F. G6 판정 | **PASS** | A안·B안 범위에서 남은 필수 결함 없음 |

> Google Drive 마운트 `G:` 에서도 `LINK=OSError WinError 1` / `RENAME_EXISTING=FileExistsError`(원본 보존)를 Codex가 직접 확인했다. Claude의 플랫폼 실측 주장이 독립 재현됐다.

### 14.2 비차단 지적 — **전부 이번 커밋에서 이행**

- **[P2]** `publish_new_note()`에서 `os.fdopen()` 자체가 실패하면 `mkstemp` fd가 열린 채 남고, Windows에서는 열린 핸들 때문에 임시 파일 삭제까지 `PermissionError`로 실패한다 (Codex 재현: `FD_LEAK=True`, `.tmp=1`).
  → **이행**: `fdopen` 실패 시 `os.close(fd)` 후 예외 재발생. 회귀 가드 `test_publish_failure_leaves_no_temp_file_behind` 추가 (총 18건). 수정을 되돌리면 정확히 이 테스트만 실패함을 확인.
- **[P2]** P1-15에 `processed_index.jsonl` sidecar의 동시 중복 append 위험 누락 → **이행**: 잔여 위험 ⑤로 추가.
- **[P3]** 강제 종료 시 `.tmp` 잔존 / POSIX에서 `mkstemp`의 0600 권한이 게시 파일에 유지 → Windows·G: 운영에는 비차단. **미이행(기록만)**.

### 14.3 AI-Slop 판정

> "이번 핵심 '경합 해소' 및 '회귀 가드 실효성' 주장은 독립 재현과 일치했습니다. 새로운 필수 과대주장은 발견하지 못했습니다."

재검수 #1~#3에서 3연속으로 성질 주장(자기치유 / 멱등 / 원자적 생성)이 반증됐던 패턴이 이번엔 재발하지 않았다.

### 14.4 G6 종결

- **P0 완료 범위 = 배관(instrumentation·인터페이스·shadow 관측)까지** — 사용자 A안 확정. Story 5/6/7의 행동 조건은 P1 이월 (P1-8~P1-14).
- **distiller 기존 병합·상태 파일의 다중 프로세스 완전 안전성 = P1-15 이월** — 사용자 B안 확정.
- 승인 플랜 `13 → 14 → G3 → 10 → 15 → 16 → 17 → G4 → 18 → 19 → G5 → 20 → 21 → G6` **전 구간 종료**.
